...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // 모든 화면에서 하단에 표시 (모바일: 카카오 버튼 위, 데스크톱: 우측 하단)
      "fixed bottom-0 right-0 z-[100] flex max-h-screen w-full flex-col p-4 pb-20 lg:pb-4 md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  // 모든 화면에서 하단에서 슬라이드 (위치가 항상 bottom이므로)
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      // 모바일: 항상 보임 (opacity-70), 데스크톱: hover 시 보임
      "absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-70 lg:opacity-0 transition-opacity hover:text-foreground hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 lg:group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600",
      className
    )}
    toast-close=""
    {...props}
