...
interface ReviewContentBasedCalendarProps {
...
export function ReviewContentBasedCalendar({
  contentItems,
  totalCount,
  startDateStr,
  endDateStr,
}: ReviewContentBasedCalendarProps) {
  // 콘텐츠 아이템을 review_registered_date로 그룹핑
  const groupedByDate = new Map<string, number>();

  contentItems.forEach((item) => {
    if (item.review_registered_date) {
      const dateStr = item.review_registered_date.split('T')[0];
  ...
  // review_registered_date가 있는 콘텐츠만
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 justify-center flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">리뷰 등록일</span>
        </div>
      </div>
      ...
      {groupedByDate.size > 0 && (
        <div className="space-y-2">
          <h3 className="font-semibold">등록일별 콘텐츠 수</h3>
              ...
              .map(([date, count]) => (
                <div key={date} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">등록된 리뷰</p>
                  </div>
      ...
      {groupedByDate.size === 0 && (
        <div className="text-center py-8 text-muted-foreground">
          <p>업로드된 콘텐츠가 없습니다.</p>
          <p className="text-sm mt-1">데이터 관리 페이지에서 엑셀을 업로드하면 리뷰등록날짜 기준으로 표시됩니다.</p>
        </div>
      )}
    </div>
  );
