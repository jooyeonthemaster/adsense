...
interface ContentBasedCalendarProps {
...
export function ContentBasedCalendar({
  contentItems,
  totalCount,
  startDateStr,
  endDateStr,
}: ContentBasedCalendarProps) {
    ...
    <div className="space-y-6">
      ...
      {groupedByDate.size === 0 && (
        <div className="text-center py-8 text-muted-foreground">
          <p>업로드된 콘텐츠가 없습니다.</p>
          <p className="text-sm mt-1">데이터 관리 페이지에서 엑셀을 업로드하면 발행일 기준으로 표시됩니다.</p>
        </div>
      )}
    </div>
  );
